...
public class CodeStyleTemplateRepository {
     ...
     * 搜索方法，返回分页 DTO
     */
    public HomePageSearchResultDTO searchByKeywordWithParams(
            String keyword,
            Pageable pageable,
        ...
        try {
            ...
            if (pageNumber < 0 || pageNumber >= totalPages || pageSize <= 0) {
                throw new RuntimeException("Repository层遭遇不合理的分页设置，当前页: "+pageNumber +
                        " 期望最大页: " + (totalPages - 1) +
                        " ;pageSize数目: " + pageSize +
                        " 当前关键词最大数目: " + total);
            }
                ...
                if (tagsAgg != null) {
                    ...
                    if (tagsAgg.aggregation().getAggregate().sterms() != null) {
                        tagsAgg.aggregation().getAggregate().sterms().buckets().array().forEach(bucket ->
                                topTags.add(bucket.key().stringValue()));
                    }
                }
            }
    ...
    private void handleAggField(NativeQueryBuilder builder) {
        int topK = properties.getAggTopKth();
        builder.withAggregation("tagsAgg",
                Aggregation.of(a -> a
                        .terms(t -> t
    ...
    private Query buildBoolQuery(String keyword, TimeRangeParamDTO timeParam) {
        ...
        MultiMatchQuery.Builder multiBuilder = new MultiMatchQuery.Builder()
                .query(keyword)
                .fields("nameCh", "nameEn", "description", "searchTags").type(TextQueryType.CrossFields);
