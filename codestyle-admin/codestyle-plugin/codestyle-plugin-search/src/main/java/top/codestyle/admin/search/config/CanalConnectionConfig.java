...
public class CanalConnectionConfig {
    ...
    public CanalConnector canalConnector() {
        log.info("初始化Canal连接器，地址: {}:{}, 实例: {}", canalProperties.getHost(), canalProperties.getPort(), canalProperties
            .getInstance());

        // 创建连接器
        CanalConnector connector = CanalConnectors.newSingleConnector(new InetSocketAddress(canalProperties
            .getHost(), canalProperties.getPort()), canalProperties.getInstance(), canalProperties.getDb()
                .getUsername(), canalProperties.getDb().getPassword());
    ...
    private String buildSubscribeExpression() {
        List<String> subscribeTables = canalProperties.getSubscribeTables();
